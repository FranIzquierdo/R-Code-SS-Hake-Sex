...
flim<-2 # valor superior a Fcrash de SS html		
mulFlim<-flim/denom # este es el multiplier para llegar a Flim.

fmult=seq(0,mulFlim+0.2,by=0.2)
...
# si fijamos las capturas estmos obligando a cada flota a que pesque un porcentaje de captura
# las flotas pescan segun su selectividad por lo que es mejor hacerlo para distintos niveles de F
...
flim<-2.5 # valor superior a Fcrash de SS html		
mulFlim<-flim/denom # este es el multiplier para llegar a Flim.

fmult=seq(0,mulFlim+0.25,by=0.25)
Fmult_names=paste0("Fmult",fmult)
l_fmult=length(fmult)
aux=df1
