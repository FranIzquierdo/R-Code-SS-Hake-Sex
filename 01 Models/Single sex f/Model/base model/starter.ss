...
#C file write time: 2021-12-21 13:01:19
shake_data.ss
control_fixed.ss
...
0 # 1 # Include prior_like for non-estimated parameters (0,1) 
...
-1 # min yr for sdreport outputs (-1 for styr); #_1946
...
5 # Annual_F_units: 0=skip; 1=exploitation(Bio); 2=exploitation(Num); 3=sum(Apical_F's); 4=true F for range of ages; 5=unweighted avg. F for range of ages
1 7 # 3 7 #_min and max age over which average F will be calculated, with F=Z-M
0 # 0 # F_std_basis: 0=raw_annual_F; 1=F/Fspr; 2=F/Fmsy; 3=F/Fbtgt; where F means annual_F; values >=11 invoke N multiyr (up to 9!) with 10's digit; >100 invokes log(ratio)
...
-1 # random number seed for bootstrap data (-1 to use long(time) as seed): # 1640336307
